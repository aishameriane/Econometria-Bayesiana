...
cut  <- 2.1      # Limite de integração
df   <- 12       # Graus de liberdade

############ Usando a densidade original ####################
t  <- rt(n = Esse, df = df)  # Amostra S valores da distibuição t
tt <- mean((t^5)*(t>cut))    # Estimativa
tp <- cumsum((t^5)*(t>cut))/den  # Evolução da estimativa à medida que S cresce


############ Usando a densidade normal ####################
z   <- rnorm(n = Esse, mean = 0, sd = 1) # Amostra S valores da normal padrão
fz  <- (z^5)*(dt(x = z, df = df)/dnorm(z))*(z>cut) # Avalia a função nas realizações
zz  <- mean(fz) # Estimativa de MC
zp  <- cumsum(fz)/den # Evolução da estimativa à medida que S cresce


############ Usando a densidade Cauchy ####################
c   <- rcauchy(n = Esse, location = 0, scale = 1) # Poderia ter usado a t com 1 gl também
fc  <- (c^5)*(dt(x=c, df = df)/dcauchy(c, location = 0, scale = 1))*(c>cut) # Avalia a função nas realizações
cc  <- mean(fc) # Estimativa de MC
cp  <- cumsum(fc)/den # Evolução da estimativa à medida que S cresce


############ Usando a densidade Cauchy porém com uma densidade t com 1 gl ####################
c1   <- rt(n = Esse, df = 1) # Cauchy é uma t com 1 gl também
...
u    <- (1/cut)*runif(n = Esse) # Amostrar da uniforme (0, 1/2.1) é igual a amostrar da uniforme padrão e dividir por 2.1
u7   <- u^(-7)
tu   <- dt(1/u, df = df)
fu   <- u7*tu                   # Avalia a função nas realizações
uu   <- mean(fu)                # Estimativa de MC
up   <- cumsum(fu)/(cut*den)    # Evolução da estimativa à medida que S cresce
...
p <- ggplot(df2, aes(Densidade, value, colour = variable)) +
  ...
  labs(title="", y = "Estimativa", x= "Iterações (x 10^5)", color = "Densidade")+
  scale_colour_brewer(palette = "Set1") +
  theme_bw()
